...
However, when you have chained operations, the pipe operator can make the code more readable. For a realistic example, see [R4DS on the pipe](https://r4ds.hadley.nz/data-transform.html#sec-the-pipe), where they show a dramatic gain in readability from using the pipe. To run their example, you will need to have the `flights` dataset from the `nycflights13` package and the `tidyverse` package attached.
