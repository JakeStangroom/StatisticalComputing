# Figures with GGPlot {#sec-day-5-gg}

Continuing with expanding our understanding of the `tidyverse`, we can consider the approach to visual representations and plots in *R*. The package responsible for plotting within the tidyverse framework is `ggplot2`.
...
For this reason, we will be using `ggplot2` for our data visualisation needs. `ggplot2` is a powerful and flexible package that allows you to create a wide variety of plots. Recall that tidyverse tries to create consistency in their package philosophies and grammar. This will close the loop for the toolkit of a modern statistician in terms of data in *R.*
...
`ggplot2` has an underlying philosophy, the Grammar of Graphics, which is a systematic way of thinking about visualisation. This grammar is based on three things:

1.  **Data:** we want *tidy data!*
2.  **Aesthetics (`aes`):** this represents how variables map to visual properties.
3.  **Geometries (`geom`**): this represents the actual plotting of the data.
...
-   **Initiating a Plot**:\
    Use `ggplot(data, aes(x, y))` to start a plot. This sets up the canvas by specifying the data and how variables are mapped to the x and y axes. Note that without specifying any `geom`s (see below), you will not see any content:
...
To include a figure that is cross-referenceable, I usually save the image first to file and then use Quarto (or `LaTeX` syntax) to include it with a caption, label and cross-reference. Even if I don't want a cross-referencable figure, I usually do this, as I find that the figure dimensions and proportions are more easily controlled this way (than just outputting the figure directly from a code chunk).
